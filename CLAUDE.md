...
Verified: **83 unit/integration tests**, `tsc`, `vite build`, and multiple real-browser runs (build via tools, route to win, reload/resume; splitters, tunnels, reset, click-belts each proven).

Next: **multi-target progression** (fill the bar → advance to a bigger reachable number → grant more operators/nodes — now reachable via splitters/tunnels); then − / × / ÷ operators as editable data; procedural deposits beyond the origin chunk; drag-to-pan; and an adversarial code-review pass over the whole rework.
