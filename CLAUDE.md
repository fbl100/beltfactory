...
- **World layers (all sparse Maps in `GameState`):** 1×1 `belts`, 1×1 `splitters`, 1×1 `tunnels`, 3×3 `buildings` (miner/operator/target, keyed by anchor) over a `nodes` ground layer + a derived `occupancy` index. `src/sim/buildings.ts` owns 3×3 geometry (footprint, ports, rotation, `minerOutputs`).
- **Machines:** miner is a wide source (emits from all 4 sides, 12 cells); operator takes two labeled inputs (**A**, **B**) on the sides flanking its front and outputs **A op B** at the front (back side reserved for a future 2nd output); target hub needs the level's delivery count (progress bar), wins on exact value, flashes "Not yet" on a wrong one.
...
- **UX:** tools Belt/Split/Tunnel/Miner/+Op (hotkeys 1–5), R rotate, right-drag erase, placement ghost, **Reset** button; save is **v3** (belts/splitters/tunnels/buildings/nodes/delivered) and self-heals old/partial saves; Docker Compose, seeded-user login, per-user JSON save/resume, pan/zoom + chunk streaming, 3 live themes, autosave.

- **Multi-target progression (shipped):** filling a level's delivery bar auto-advances the *same* factory to a bigger target and grants a new number deposit; the final level wins. The ladder lives in `src/content/levels.ts` (editable data — `LEVELS` is the single source of truth). `src/sim/progression.ts` owns `advanceLevel` (run after `move()` each tick), `reconcileLevel` (LEVELS drives the hub on load; migrates v3→v4 saves in place), buried-deposit relocation, and stale-value handling (leftover old-target output isn't flagged "Not yet"). `GameState.levelIndex`; save **v5** (v3/v4 still load). HUD shows "Level N/M" + an auto-advance toast; the camera nudges to a newly-revealed deposit if off-screen.
- **Prime Foundry ladder (shipped, playtested):** the level data is a prime-factorization curve — deposits are **primes**, targets are composites you build by multiplying them. It stays on {2,3} for a while so she gets fluent — 6=2×3 → 12=2×2×3 (reuse the 2) → 21=2×3×3+3 (× and + together) — then introduces one prime at a time: 30=2×3×5 → 42=2×3×7 → 210=2×3×5×7 (finale, 6 levels). Deposits are spread out (2 top-left, 3 bottom-left ~10 apart, later primes across the bottom) to keep the middle open and belts from turning to spaghetti. `×` is essential (additive route explodes) while addition stays possible so she's never stuck. Tests guard the invariants (deposits prime; each target buildable with the level ops in a few machines; still addition-reachable).

- **Operators − / × / ÷ (shipped):** `src/content/operations.ts` has add/subtract/multiply/divide, all *order-independent* (− = |a−b|, × = a·b, ÷ = bigger÷smaller whole part, ÷0→0) so the two-input operator machine needs no operand ordering and a 9-year-old never meets negatives or fractions. `Level.ops` gates which the player may build (Prime Foundry offers +/× from the start; the finale unlocks all four); the HUD shows a level-gated op-type selector. The two inputs are **labeled ports A and B** (the sides flanking the front output; `operatorSides()`), with the back side reserved for a future second output (e.g. a divisor emitting quotient + remainder — the "remainder crumbs" idea). It renders `A×B` in the body with `A`/`B` at the input ports. It holds at most **one pending input per side** (`OperatorInput{side,value}`), so two items from the *same* belt can't pair (that had produced e.g. 3×3=9 instead of 2×3=6); the entry side is opposite the item's travel direction. Pending inputs are transient (reset on load).
