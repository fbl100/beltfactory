...
- **Machines:** miner is a wide source (emits from all 4 sides, 12 cells) and is now **auto-placed on every number deposit** (`ensureMiners` in `world.ts`, run on new game / level-up / load; a miner overrides any belts under its footprint; miners are permanent — the eraser can't remove them). There is no manual Miner tool. Operator is a **1×3 bar** — its two end cells are labeled input tips (**A**, **B**; any exposed edge accepts) and its center outputs **A op B** from either of its two long edges (the facing side preferred, the other a fallback); target hub needs the level's delivery count (progress bar), wins on exact value, flashes "Not yet" on a wrong one.
...
- **UX:** tools Belt/Split/Tunnel/Erase + the op-type buttons that build operators (hotkeys 1/4/5/6 + op buttons; miners are automatic so there's no Miner tool), R rotate, **Eraser** tool *or* right-drag to erase (belts/splitters/tunnels/operators — miners & goal are protected), placement ghost, **Clear Map** button (wipes this level's belts + operators but keeps the level, target hub, deposits, and their auto-miners; `clearBuild` in `world.ts`) + **Start Over** button (full restart to level 1; `resetGame`); gentle pinch-zoom. Save is **v5** and self-heals old/partial saves; Docker Compose, seeded-user login (`abby`/`apples` seeded by default), per-user JSON save/resume, pan/zoom + chunk streaming, 3 live themes (**Neon Arcade** default), autosave.
