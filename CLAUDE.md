...
- **Multi-target progression (shipped):** filling a level's delivery bar auto-advances the *same* factory to a bigger target and grants a new number deposit; the final level wins. The ladder lives in `src/content/levels.ts` (editable data — `LEVELS` is the single source of truth; targets 12→20→30→50→100, gentle "add the new number" then bold "double it": 50=25+25, 100=50+50 via the wide miner). `src/sim/progression.ts` owns `advanceLevel` (run after `move()` each tick), `reconcileLevel` (LEVELS drives the hub on load; migrates v3→v4 saves in place), buried-deposit relocation, and stale-value handling (leftover old-target output isn't flagged "Not yet"). `GameState.levelIndex`; save **v4**. HUD shows "Level N/M" + an auto-advance toast; the camera nudges to a newly-revealed deposit if off-screen.

Verified: **101 unit/integration tests**, `tsc`, `vite build`, and multiple real-browser runs (build via tools, route to win/advance, reload/resume; splitters, tunnels, reset, click-belts, and level-up→toast→new-deposit each proven). Progression was designed via an adversarial design panel and reviewed via an adversarial code-review workflow (findings fixed).

Next: − / × / ÷ operators as editable data (wire `Level.ops`); procedural deposits beyond the origin chunk; drag-to-pan; retune/extend the level ladder for her actual pace.
