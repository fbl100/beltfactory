...
- **Machines:** miner is a wide source (emits from its 3 open sides, 9 cells); operator takes inputs on its 3 non-front sides and outputs at the front center; target hub needs `TARGET_COUNT` deliveries (progress bar), wins on exact value, flashes "Not yet" on a wrong one.
